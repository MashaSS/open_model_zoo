...
std::shared_ptr<InternalModelData> HpeAssociativeEmbedding::preprocess(const InputData& inputData, InferRequest::Ptr& request) {
    ...
    if (inputLayerSize.height - stride >= roi.height
        ...
        slog::info << "    image: " << image.cols << "x" << image.rows << slog::endl;
        slog::info << "    paddedImage: " << paddedImage.cols << "x" << paddedImage.rows << slog::endl;
        slog::info << "    inputLayerSize: " << inputLayerSize.width << "x" << inputLayerSize.height << slog::endl;
        slog::info << "    roi: " << roi.width << "x" << roi.height << slog::endl;
    }
